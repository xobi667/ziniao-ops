...
1. In normal Codex Browser work, take the current visible Browser URL and query the static action memory. This does not scan local Chrome/Edge/Ziniao windows and does not control Browser:
...
Only after the user explicitly authorizes the legacy local bridge, list what an already-open local page has in memory:
...
`list-xinjian-page-actions.ps1` resolves an already-open local 心舰 URL read-only, then returns the remembered page, every mapped action on that page, each action's purpose, safety mode, locator strategy, `rpa_command_mode`, and exact `rpa_commands` from `xinjian-ui-rpa-command-inventory.json`. Prefer the listed `exact_query`, `dry_run`, `execute_safe`, `read_table_column`, `row_context`, `confirmed_write`, or `confirmed_export` command fields over fuzzy intent routing when an action is already visible in this list. Read top-level `current_url`, `current_title`, `resolved_port`, `page_kind`, `command_inventory`, `login_bridge`, and `next_action` first; login/no-access pages return those fields even when no actions are listed. By default it never starts a local login bridge: when no page is resolved it returns `mode=codex_browser_handoff` and `next_action=use_codex_builtin_browser`. Every explicitly authorized legacy local-list run must pass `-AllowLegacyLocalBridge`; `-Port` merely pins that legacy diagnostic to a specific port. Pass `-Url "<当前心舰URL>"` to override legacy detection, `-Intent "<用户要做什么>"` to disambiguate local windows, or `-SafeOnly` for safe non-write actions only.
...
`learn-xinjian-current-page.ps1` is a legacy local-CDP learner. With explicit `-AllowLegacyLocalBridge`, it resolves the current page, auto-selects a reachable debuggable 心舰 port, captures sanitized controls, and regenerates public maps/catalog. Ordinary `-DryRun` still resolves the local page and capture port, so it also requires the gate. `-GenerateOnly` is the offline exception and only rebuilds maps/catalog from saved sanitized captures. Without the gate, any non-`GenerateOnly` run returns `codex_browser_handoff` with exit 2 before URL/window/CDP detection. Authorized captures still validate the matched route and do not read cookies, storage, tokens, input values, or table row cell values.
...
`learn-xinjian-open-pages.ps1` enumerates already open reachable DevTools 心舰 pages only after explicit legacy authorization, de-duplicates by route, and passes `-AllowLegacyLocalBridge` to every current-page learner. `-Port` never substitutes for the gate. A no-URL `-DryRun` still enumerates local CDP pages and therefore remains gated. The only ungated form is a pure offline plan with one or more explicit `-Url` values plus `-DryRun`; it validates and de-duplicates those URLs without probing local windows or ports.
...
The local invoker reports the matched page/action, safety gate, locator strategy, `execution_backend`, and top-level `current_url`, `current_title`, `resolved_port`, `page_kind`, and `login_bridge`. It is a legacy local CDP/UIA executor and must not be used to control a Codex built-in Browser tab. Without `-AllowLegacyLocalBridge`, an execute request that lacks a controllable local page returns `mode=codex_browser_handoff` and never calls `open-xinjian-login.ps1`. Use `query-xinjian-ui-action.ps1` for Browser-side read-only planning, then perform the actual click with Browser. Only after explicit legacy authorization may the invoker scan local DevTools ports, open the manual bridge, and execute with `-Execute -AllowLegacyLocalBridge`; write/delete/save/submit/export actions still require the exact `-AllowWrite` or `-AllowExport` confirmation.

For export/download actions, the invoker now returns a machine-readable `post_execute` object. If export is not authorized yet, `post_execute.rerun_after_confirmation` contains the exact execution flags (`execute=true`, `allow_legacy_local_bridge=true`, `allow_export=true`) required after explicit user confirmation. If an authorized export execution succeeds, top-level `next_action` is `wait_for_xinjian_export_or_open_download_center`; first run `scripts/wait-xinjian-export.ps1` to watch Downloads and analyze supported exports, then fall back to the mapped `打开下载中心` action if 心舰 creates an async report in the download center instead of a direct browser download.

For write/delete/save/submit actions, `post_execute.kind` is `write_confirmation_follow_up`. If the action is not authorized yet, `post_execute.rerun_after_confirmation` contains the exact execution flags (`execute=true`, `allow_legacy_local_bridge=true`, `allow_write=true`) required after explicit user confirmation. If an authorized write execution succeeds, top-level `next_action` is `verify_xinjian_page_state_after_write`; first run `scripts/list-xinjian-page-actions.ps1 -AllowLegacyLocalBridge` to verify the current legacy page/action state, then run `scripts/learn-xinjian-current-page.ps1 -AllowLegacyLocalBridge` only if the confirmed action changed visible dialogs, drawers, or page controls that should be remembered.

Within an explicitly authorized legacy run, when no debuggable CDP port is resolved the invoker can still execute mapped safe non-write controls through Windows UI Automation if the already-open 心舰 window exposes a matching UIA element. In that case dry-run reports `execution_backend = "uia"`, `uia_fallback_available = true`, and `can_execute = true`; `-Execute -AllowLegacyLocalBridge` invokes the UIA `InvokePattern` or `SelectionItemPattern` without moving the mouse or opening a duplicate browser. UIA fallback is deliberately narrow: write/export actions, row-context actions, and unknown-safety actions remain blocked. Read-only table-column memory needs a debuggable CDP route because it reads visible DOM table cells instead of clicking UIA controls.

Locator strategy is layered. Exact selectors, row-action locators, hrefs, visible DOM text, placeholder strings, placeholder lists, and known tab-text lists are used first. For known date/platform lists, the invoker passes the user's original intent into the CDP helper so requests such as `近7天`, `最近七天`, `Shopee`, or `Lazada` can choose the matching visible tab instead of clicking a generic label. When a safe tab/status-tab has no explicit locator, the invoker may click the matching visible action text. When a date filter has no explicit placeholder, it may focus the nearest visible date/select control next to the filter label. Observed table metric/header entries such as `ROAS`, `广告花费`, and `转化率` are recorded as read-only `table_column` actions with `read_table_column_header`; they are not clicked, but in an explicitly authorized legacy run the invoker can read visible values under the remembered column with `-Execute -AllowLegacyLocalBridge` when the current page is debuggable through CDP. If `-RowIndex`, `-RowText`, or a clear row phrase such as `第1行广告花费` / `包含 xxx 的行ROAS` is provided, the table-column read is filtered to that visible row and returns the original 1-based row index. Row-level entries such as `行分析`, `操作`, `详情`, `编辑`, `删除`, `分配`, or `认领` require resolved row context. Pass `-RowIndex <1-based row number>` / `-RowText "<visible row text>"`, or include a clear row phrase in the intent such as `第1行编辑`, `第一行详情`, or `包含 xxx 的行编辑`; the invoker refuses to blindly click the first row. Explicit row arguments override inferred row context. Write/export row actions still require `-AllowWrite` or `-AllowExport`.

If a row-level action is matched without row context, the invoker returns `row_context_follow_up.kind = row_context_required_follow_up`. The object contains `rerun_with_row_index` and `rerun_with_row_text` command fields that preserve `allow_legacy_local_bridge=true`, plus `additional_confirmation_required` when the row action is also write/export protected. Use that object as the next step instead of guessing the target row from screenshots or clicking the first visible row.
...
The following live exercise and audit commands are legacy local-CDP diagnostics. They require explicit user authorization and `-AllowLegacyLocalBridge`; without it they return the Codex Browser handoff before any CDP/UIA probe. Their offline `-DryRun` and `-UseExistingCaptures` modes do not require the gate.
...
- `invoke-xinjian-ui-action.ps1` is dry-run by default. Only an explicitly authorized legacy run with `-AllowLegacyLocalBridge` may use `-Execute` to click safe non-write actions through CDP, or through Windows UI Automation when the current non-debuggable 心舰 window has a matching mapped UIA control. Confirmation-required write/export actions additionally require `-AllowWrite` or `-AllowExport`, and UIA fallback never broadens those permissions.
...
- Live button coverage audit: 48 catalog routes selected, 47 live pages captured, 0 business no-access pages, 1 restricted terminal page, 827 visible business controls observed, 827 matched, 0 missing controls.
...
- RPA readiness: catalog quality has 0 missing purpose, 0 missing function source, 0 missing context, 0 manual-review/map-only/empty-locator actions. Remaining non-default boundaries are 83 confirmation-required write actions, 9 confirmation-required export actions, and 12 row-context actions that require explicit or inferred row context. The 460 read-only table-column memories are tracked as non-gap readable column memory; `/index/noaccess` is tracked separately as a non-gap restricted terminal page.
