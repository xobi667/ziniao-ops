...
- Use the Codex built-in Browser as the default web interaction surface in both Codex CLI and Codex IDE.
- Use this sanitized catalog or `query-xinjian-ui-action.ps1` for action planning, then let the built-in Browser inspect and operate the page.
- `invoke-xinjian-ui-action.ps1` is an explicitly authorized legacy local-bridge invoker only; every such invocation requires `-AllowLegacyLocalBridge`.
- Write, export, and manual-review actions still require explicit confirmation for the exact action.
