...
if (!$OutputDir) {
  $OutputDir = Join-Path $root ".ziniao-ops\xinjian-dialog-captures"
}
New-Item -ItemType Directory -Force -Path $OutputDir | Out-Null
if (!$StatePath) {
  $StatePath = Join-Path $root ".ziniao-ops\xinjian-dialog-crawl-state.json"
}
...
function Get-ExistingDialogPaths {
  $paths = @{}
  $dialogMapPath = Join-Path $root "references\xinjian-ui-dialog-map.json"
  if (!(Test-Path -LiteralPath $dialogMapPath)) { return $paths }
  try {
    $map = Get-Content -LiteralPath $dialogMapPath -Raw -Encoding UTF8 | ConvertFrom-Json
    foreach ($page in @($map.pages)) {
...
$existingDialogPaths = if ($OnlyMissing) { Get-ExistingDialogPaths } else { @{} }
$routes = @()
foreach ($page in @(Get-MapPages)) {
  ...
  if ($OnlyMissing -and $existingDialogPaths[$key]) { continue }
...
foreach ($route in $routes) {
  ...
  try {
    $argsList = @(
      ...
      "-File", (Join-Path $PSScriptRoot "capture-xinjian-dialogs.ps1"),
      "-Port", [string]$Port,
      "-Url", $url,
  ...
  if ($capture -and $capture.ok) {
    ...
    $status = if ($capturedKey -and $capturedKey.ToLowerInvariant() -ne $routeKey.ToLowerInvariant()) {
      ...
      "no_safe_dialog_triggers"
    } elseif ([int]$capture.counts.dialogs -le 0) {
      "triggers_without_dialogs"
    } else {
      "captured"
    }
    Set-CrawlAttempt -State $state -AttemptMap $attemptMap -Route $path -Url $url -Status $status -Data ([pscustomobject]@{
        ...
        triggers_with_dialogs = $capture.counts.triggers_with_dialogs
        dialogs = $capture.counts.dialogs
        dialog_buttons = $capture.counts.dialog_buttons
        field_labels = $capture.counts.field_labels
...
if ($Json) {
  ...
  Write-Host ("Captured dialogs on {0}/{1} Xinjian pages." -f (@($captures | Where-Object { $_.ok }).Count), $captures.Count)
  foreach ($item in $captures) {
    if ($item.ok) {
      Write-Host ("OK {0} -> {1} triggers, {2} dialogs ({3})" -f $item.route, $item.triggers, $item.dialogs, $item.status)
    } else {
