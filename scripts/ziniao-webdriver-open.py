...
def _get_browser_list(port: int, config: dict) -> tuple[list[dict], dict | None]:
    payload = _request_payload("getBrowserList", config)
...
def _classify_empty_browser_list(
    ...
    return {
        "error": "ziniao_webdriver_not_ready",
...
def _start_browser(port: int, config: dict, browser: dict, timeout: int = 90) -> dict | None:
    store_id = str(browser.get("browserOauth") or browser.get("browserId") or "").strip()
    payload = _request_payload("startBrowser", config)
    payload.update(
        {
...
def main() -> int:
    ...
    if not browsers:
        ...
        failure = _classify_empty_browser_list(raw, start_mode, auth_info)
        waited_seconds = 0 if (
            blocked or start_mode == "existing_webdriver_login_error_no_visible_login"
        ) else (args.login_timeout if login_error_seen else 25)
        _print_json(
            {
                "ok": False,
                "method": "ziniao_webdriver",
                "error": raw.get("error") if blocked and isinstance(raw, dict) else failure["error"],
                "message": raw.get("message") if blocked and isinstance(raw, dict) else failure["message"],
                "raw_status": raw,
    ...
    opened = _start_browser(port, config, browser)
