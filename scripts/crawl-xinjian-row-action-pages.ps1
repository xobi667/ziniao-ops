...
if (!$OutputDir) {
  $OutputDir = Join-Path $root ".ziniao-ops\xinjian-row-action-captures"
}
New-Item -ItemType Directory -Force -Path $OutputDir | Out-Null
if (!$StatePath) {
  $StatePath = Join-Path $root ".ziniao-ops\xinjian-row-action-crawl-state.json"
}
...
function Get-ExistingRowActionPaths {
  $paths = @{}
  $mapPath = Join-Path $root "references\xinjian-ui-row-action-map.json"
  if (!(Test-Path -LiteralPath $mapPath)) { return $paths }
  try {
...
$existingPaths = if ($OnlyMissing) { Get-ExistingRowActionPaths } else { @{} }
$routes = @()
foreach ($page in @(Get-MapPages)) {
  ...
  if ($OnlyMissing -and $existingPaths[$key]) { continue }
...
foreach ($route in $routes) {
  ...
  try {
    $argsList = @(
      ...
      "-File", (Join-Path $PSScriptRoot "capture-xinjian-row-actions.ps1"),
      "-Port", [string]$Port,
      "-Url", $url,
      "-OutputPath", $capturePath,
      "-MaxTables", [string]$MaxTables,
      "-MaxRowsPerTable", [string]$MaxRowsPerTable,
      "-Json"
    )
  ...
  if ($capture -and $capture.ok) {
    ...
    $status = if ($capturedKey -and $capturedKey.ToLowerInvariant() -ne $routeKey.ToLowerInvariant()) {
      "redirected"
    } elseif ([int]$capture.counts.tables -le 0) {
      "no_tables"
    } elseif ([int]$capture.counts.row_actions -le 0) {
      "tables_without_row_actions"
    } else {
      "captured"
    }
    Set-CrawlAttempt -State $state -AttemptMap $attemptMap -Route $path -Url $url -Status $status -Data ([pscustomobject]@{
        ...
        tables = $capture.counts.tables
        row_actions = $capture.counts.row_actions
        sampled_rows = $capture.counts.sampled_rows
...
if ($Json) {
  ...
  Write-Host ("Captured row actions on {0}/{1} Xinjian pages." -f (@($captures | Where-Object { $_.ok }).Count), $captures.Count)
  foreach ($item in $captures) {
    if ($item.ok) {
      Write-Host ("OK {0} -> {1} tables, {2} row actions ({3})" -f $item.route, $item.tables, $item.row_actions, $item.status)
    } else {
