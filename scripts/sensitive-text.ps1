...
function Get-ZiniaoOpsSecretRegex {
  return [regex]("(?i)(" + (Get-ZiniaoOpsSecretKeywordPattern) + ')\s*(?:[:=\uFF1A]{1,2}|\u662F|\u4E3A|\u53EB|\u5982\u4E0B|\bis\b|\bare\b|\bwas\b)\s*[^\s,;\uFF0C\uFF1B\u3002]+')
}

function Get-ZiniaoOpsSecretWhitespaceRegex {
  $keyword = Get-ZiniaoOpsSecretKeywordPattern
  $value = '(?:\d{4,8}|(?=[A-Za-z0-9._~+/=-]{6,}\b)(?=[A-Za-z0-9._~+/=-]*[A-Za-z])(?=[A-Za-z0-9._~+/=-]*\d)[A-Za-z0-9._~+/=-]{6,}|[A-Za-z0-9._~+/=-]{16,})'
...
function Test-ZiniaoOpsSecretText {
  ...
  return ((Get-ZiniaoOpsSecretRegex).IsMatch($Text) -or (Get-ZiniaoOpsSecretWhitespaceRegex).IsMatch($Text))
}
