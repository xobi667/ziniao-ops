...
`list-xinjian-page-actions.ps1` resolves the current 心舰 URL from visible/debuggable windows read-only, then returns the remembered page, every mapped action on that page, each action's purpose, safety mode, locator strategy, `rpa_command_mode`, and exact `rpa_commands` from `xinjian-ui-rpa-command-inventory.json`. Prefer the listed `exact_query`, `dry_run`, `execute_safe`, `read_table_column`, `row_context`, `confirmed_write`, or `confirmed_export` command fields over fuzzy intent routing when an action is already visible in this list. Read top-level `current_url`, `current_title`, `resolved_port`, `page_kind`, `command_inventory`, `login_bridge`, and `next_action` first; login/no-access pages return those fields even when no actions are listed. By default it scans all reachable Chrome/Edge/Ziniao DevTools ports and returns `resolved_port` when a debuggable tab is selected; if no current 心舰 URL is resolved, it automatically calls `open-xinjian-login.ps1` and reports whether it reused an existing page or opened a debuggable manual-login bridge. Pass `-Port` only to pin diagnostics to a specific port. Pass `-Url "<当前心舰URL>"` to override detection, `-Intent "<用户要做什么>"` to disambiguate multiple open 心舰 windows, `-SafeOnly` for safe non-write actions only, or `-NoAutoOpenLogin` to suppress bridge opening during diagnostics.
...
The invoker reports the matched page/action, safety gate, locator strategy, `execution_backend`, and top-level `current_url`, `current_title`, `resolved_port`, `page_kind`, and `login_bridge`. If `-Url` is omitted, it detects visible/debuggable 心舰 windows read-only and scores candidate URLs against the user's intent; a single best visible match becomes the current page. If no 心舰 page is resolved, or an explicit execute request names a URL while no 心舰 window is detected at all, it automatically calls `open-xinjian-login.ps1`; login pages stop with `manual_login_required_in_debuggable_xinjian_browser` until the employee logs in manually. By default it scans reachable DevTools ports instead of assuming `9342`; pass `-Port` only to pin execution to a specific browser. Pass `-Url "<当前心舰URL>"` to override detection, `-NoAutoDetectUrl` to force global matching, or `-NoAutoOpenLogin` to suppress the login bridge in tests. It only clicks when `-Execute` is passed. Write/delete/save/submit/export actions stay blocked unless the exact operation is explicitly allowed with `-AllowWrite` or `-AllowExport`.
...
For write/delete/save/submit actions, `post_execute.kind` is `write_confirmation_follow_up`. If the action is not authorized yet, `post_execute.rerun_after_confirmation` contains the exact execution flags (`execute=true`, `allow_write=true`) required after explicit user confirmation. If an authorized write execution succeeds, top-level `next_action` is `verify_xinjian_page_state_after_write`; first run `scripts/list-xinjian-page-actions.ps1` to verify the current page/action state, then run `scripts/learn-xinjian-current-page.ps1` only if the confirmed action changed visible dialogs, drawers, or page controls that should be remembered.
